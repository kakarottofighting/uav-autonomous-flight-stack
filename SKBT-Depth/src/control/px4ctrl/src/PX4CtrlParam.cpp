...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle &nh)
{
	...
	nh.param("nmpc/enable", nmpc.enable, false);
	nh.param("nmpc/horizon", nmpc.horizon, 10);
	nh.param("nmpc/gradient_steps", nmpc.gradient_steps, 4);
	nh.param("nmpc/dt", nmpc.dt, 0.02);
	nh.param("nmpc/alpha", nmpc.alpha, 0.05);
	nh.param("nmpc/q_pos", nmpc.q_pos, 8.0);
	nh.param("nmpc/q_vel", nmpc.q_vel, 2.0);
	nh.param("nmpc/r_acc", nmpc.r_acc, 0.08);
	nh.param("nmpc/r_delta_acc", nmpc.r_delta_acc, 0.15);
	nh.param("nmpc/max_acc", nmpc.max_acc, 5.0);
	nh.param("nmpc/min_thrust", nmpc.min_thrust, 0.05);
	nh.param("nmpc/max_thrust", nmpc.max_thrust, 0.90);
