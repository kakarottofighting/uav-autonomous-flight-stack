...
private:
  ...
  Eigen::Vector3d solveNmpcAcceleration(const Desired_State_t &des, const Odom_Data_t &odom);
  Eigen::Vector3d projectControlAcceleration(const Eigen::Vector3d &acc) const;
  Eigen::Vector3d getReferencePosition(const Desired_State_t &des, double t) const;
  Eigen::Vector3d getReferenceVelocity(const Desired_State_t &des, double t) const;
  double clampValue(double value, double min_value, double max_value) const;

  Eigen::Vector3d last_nmpc_acc_ = Eigen::Vector3d::Zero();
  bool nmpc_warm_started_ = false;
};
