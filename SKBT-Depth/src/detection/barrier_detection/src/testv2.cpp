...
int find_most_depth(int l_depth, const std::vector<std::vector<cv::Point>>& h_points, const std::vector<bool>& h_flags){
    //统计最多的点集
    int m = 1;
    for (size_t i = l_depth + 40; i < h_points.size(); ++i) {
        if(h_flags[i] == true){
            if (h_points[i].size() > h_points[m].size()) {
            m = i;
        }
        }   
    }
...
for (int i = 0; int(i < depth_image_noinf.rows * 4 / 5); ++i){
    for (int j = 0; j < depth_image_noinf.cols; ++j){
        float depth = depth_image_noinf.at<float>(i, j);
        if(depth == 100.0){
            blank.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 255, 0);
            blank_gray.at<uchar>(i, j) = 0;
        }
        else{
            blank.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 0, int(depth / 5 * 255));
            blank_gray.at<uchar>(i, j) = int(depth / 5 * 255);
        }
    }
}
    ...
    std::vector<bool> hist_flags(256);
    hist_flags.assign(256, true);
    for (int i = 0; i < 256; ++i) {
    ...
    for(int i = 0; i <= 1; ++i){
        ...
        int max_pix = find_most_depth(last_depth, hist_points, hist_flags);
        ...
        for (int i = max_pix + 1; i < 256 ; ++i ){
            //if(hist_points[i].size() - hist_points[i - 1].size() > 100 || hist_points[i].size() == 0){
            if(hist_points[i].size() < 50){
                Up_pix = i - 1;
                break;
            }
        }

        for (int i = max_pix - 1; i > 0 ; --i ){
            //if(hist_points[i].size() - hist_points[i + 1].size() > 100 || hist_points[i].size() == 0){
            if(hist_points[i].size() < 50){
                Low_pix = i + 1;
                break;
        ...
        last_depth = Up_pix;
        //std::cout << Up_pix << " " << Low_pix << std::endl;
        //std::cout << last_depth << std::endl;
        for (int i = Low_pix; i <= Up_pix; ++i){
            hist_flags[i] = false;
