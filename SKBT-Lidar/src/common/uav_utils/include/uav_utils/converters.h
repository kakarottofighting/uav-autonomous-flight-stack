...
inline void extract_odometry(odom_fusion::OdomStampConstPtr msg, Eigen::Vector3d& p,
                      Eigen::Vector3d& v, Eigen::Quaterniond& q)
{
    ...
    v(0) = msg->pose.twist.twist.linear.x;
    v(1) = msg->pose.twist.twist.linear.y;
    v(2) = msg->pose.twist.twist.linear.z;

    q.w() = msg->pose.pose.pose.orientation.w;
    q.x() = msg->pose.pose.pose.orientation.x;
    q.y() = msg->pose.pose.pose.orientation.y;
    q.z() = msg->pose.pose.pose.orientation.z;
}
                      ...
                      Eigen::Vector3d& v, Eigen::Quaterniond& q, Eigen::Vector3d& w)
{
    ...
    w(0) = msg->pose.twist.twist.angular.x;
    w(1) = msg->pose.twist.twist.angular.y;
    w(2) = msg->pose.twist.twist.angular.z;
}
